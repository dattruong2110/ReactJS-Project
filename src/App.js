...
function App() {
  ...
  const searchData = (searchKeys) => {
    const items = data.filter(
      (item) =>
        item.name.toLowerCase().includes(searchKeys) ||
        item.name.toUpperCase().includes(searchKeys)
    );
    setMyData(items);
  };
          ...
          <Routes>
            ...
            {data.map((item) => (
              <Route
                ...
                element={<Content item={item} items={itemDetails} />}
              />
            ))}
            <Route
              path="/store-system"
              element={<StoreSystem store={myStore} />}
            />
          </Routes>
        </div>
      </div>
